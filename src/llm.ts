...
function tryParseJsonMaybe(s: string | null | undefined) {
  ...
  const cleaned = trimmed.replace(/^\s*```(?:json|html)?\s*/i, '').replace(/\s*```\s*$/i, '').trim();
...
export class OpenAIResponsesClient implements LLMClient {
  ...
  private async callResponsesAPI(payload: any): Promise<string> {
    ...
    if (Array.isArray(data.output)) {
      return data.output.map((o:any) => {
        if (typeof o === 'string') return o;
        if (o?.content && typeof o.content === 'string') return o.content;
        if (o?.text) return o.text;
        if (Array.isArray(o?.content)) return o.content.map((c:any)=>safeTrim(c?.text || c?.content || c)).filter(Boolean).join('\n');
        return '';
      }).filter(Boolean).join('\n').trim();
    }
    if (Array.isArray(data?.choices) && data.choices.length) {
      ...
      if (c?.message?.content) {
        ...
        if (Array.isArray(c.message.content)) {
          return c.message.content.map((x:any)=>safeTrim(x?.text || x)).filter(Boolean).join('\n').trim();
        }
      }
  ...
  async generateOutline(brief: any) {
    ...
    try {
      const raw = await this.callResponsesAPI({ model: this.model, input: prompt, max_output_tokens: 800 });
      ...
      return {
        outline: (brief?.outline || []).slice(0,6).map((s:any,i:number)=>({
  ...
  async generateSection(h2: string, context: any) {
    ...
    try {
      const raw = await this.callResponsesAPI({ model: this.model, input: prompt, max_output_tokens: 1200 });
      ...
      else if (/<\s*h[1-6]|<\s*p|<\s*div|<\s*pre|<\s*code/si.test(cleaned)) htmlOut = cleaned;
      else htmlOut = `<p>${safeTrim(cleaned)}</p>`;
      if (!htmlOut) htmlOut = `<h2>${h2}</h2><p>(empty model output)</p>`;
      const word_count = htmlOut.replace(/<[^>]+>/g,' ').split(/\s+/).filter(Boolean).length;
...
export class ClaudeClient implements LLMClient {
  ...
  private async callClaude(prompt: string) {
    const res = await fetch('https://api.anthropic.com/v1/complete', {
      ...
      body: JSON.stringify({ model: this.model, prompt, max_tokens: 800, temperature: 0.2 })
    });
    if (!res.ok) {
  ...
  async generateOutline(brief: any) {
    ...
    try {
      ...
      const parsed = tryParseJsonMaybe(cleaned) || tryParseJsonMaybe(String(raw));
      if (parsed && parsed.outline) return parsed;
  ...
  async generateSection(h2: string, context: any) {
    ...
    try {
      ...
      let htmlOut = '';
      if (parsed && parsed.html) htmlOut = safeTrim(parsed.html);
      else if (String(cleaned).match(/<\s*p|<\s*h|<\s*div|<pre|<code/si)) htmlOut = String(cleaned).trim();
