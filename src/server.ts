...
export const MOCK_MODE: boolean = typeof envMock !== 'undefined'
...
function normalizeDraftToHtml(draft: any): string {
  ...
  if (typeof draft === 'string') return draft.replace(/```(?:html|json)?/gi, '').replace(/```/g, '').trim();
  ...
  for (const c of candidateStrings) {
    if (typeof c === 'string' && c.trim().length > 0) return (c as string).replace(/```(?:html|json)?/gi, '').replace(/```/g, '').trim();
  }

  // If draft.sections or similar exists (array)
  const sections = draft.sections || draft.sections_html || draft.blocks || null;
  if (Array.isArray(sections) && sections.length) {
    return sections.map((s: any) => {
      if (!s) return '';
      if (typeof s === 'string') return `<p>${escapeHtml(s)}</p>`;
      const heading = s.heading || s.title || s.h || '';
      const body = (typeof s.html === 'string' && s.html) || (typeof s.body === 'string' && s.body) || s.text || '';
      const bodyStr = typeof body === 'string' ? body : escapeHtml(JSON.stringify(body));
      return (heading ? `<h2>${escapeHtml(heading)}</h2>` : '') + `<div>${bodyStr}</div>`;
    }).join('');
  }

  // If draft.outline exists and is an array
  if (Array.isArray(draft.outline) && draft.outline.length) {
    return draft.outline.map((it: any) => {
      if (typeof it === 'string') return `<h2>${escapeHtml(it)}</h2>`;
      if (typeof it === 'object') {
        const t = it.title || it.heading || '';
        const s = (typeof it.html === 'string' && it.html) || it.summary || it.snippet || it.text || '';
        const sStr = typeof s === 'string' ? s : escapeHtml(JSON.stringify(s));
  ...
  if (Array.isArray(draft)) {
    return draft.map(item => {
      ...
      if (typeof item === 'object') {
        ...
        const bStr = typeof b === 'string' ? b : escapeHtml(JSON.stringify(b));
  ...
  if (typeof draft === 'object') {
    ...
    const summary = draft.summary || draft.snippet || draft.text || draft.description || '';
    if (title || summary) {
      return (title ? `<h2>${escapeHtml(title)}</h2>` : '') + `<p>${escapeHtml(typeof summary === 'string' ? summary : JSON.stringify(summary))}</p>`;
    }
...
let fetchWithFallback: ((url: string) => Promise<{ html: string } | string>) | null = null;
...
try {
  ...
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const briefMod = (() => { try { return require('./brief'); } catch { return null; } })() || extractMod;
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  ...
  // pick best exports
  fetchWithFallback = pickExport(fetchMod, ['fetchWithFallback', 'fastFetch', 'fetch']);
  extractStructure = pickExport(extractMod, ['extractStructure', 'buildSemanticBrief', 'extract']);
  buildSemanticBrief = pickExport(briefMod, ['buildSemanticBrief', 'buildBrief', 'brief']);
  generateOutline = pickExport(outlineMod, ['generateOutline', 'outline']);
  assembleDraft = pickExport(genMod, ['assembleDraft', 'generateDraft', 'generateSection', 'default']);
  paragraphSimilarity = pickExport(safetyMod, ['paragraphSimilarity', 'passesSimilarityThreshold']);
  ...
  console.warn('Pipeline module load encountered an error, falling back to stubs. Error:', (e && e.message) || e);
}

// Fallback stubs if not present — keep them minimal & deterministic
if (!fetchWithFallback) {
  fetchWithFallback = async (url: string) => ({ html: `<html><head><title>fetched</title></head><body><h1>Sample</h1><p>Sample paragraph from ${url}</p></body></html>` });
}
if (!extractStructure) {
  extractStructure = (html: string) => ({ paragraphs: html.replace(/<[^>]+>/g, ' ').split(/\s+/).filter(Boolean), headings: [] });
}
if (!buildSemanticBrief) {
  buildSemanticBrief = (ex: any) => ({ title: ex.headings?.[0] || 'Demo Title', outline: ['Intro', 'Main point', 'Conclusion'] });
}
if (!generateOutline) {
...
if (!assembleDraft) {
  assembleDraft = async (outline: any, brief: any) => ({
    ...
    body_html: Array.isArray(outline) ? outline.map((s: any) => `<h2>${typeof s === 'string' ? s : (s?.h2 || s?.title || 'Section')}</h2><p>Demo content</p>`).join('') : `<p>Demo output</p>`
  });
}
...
// Health endpoint
app.get('/health', (_req, res) => res.json({ ok: true, mode: MOCK_MODE ? 'mock' : 'live' }));
...
app.post('/generate', async (req, res) => {
  try {
    ...
    if (MOCK_MODE) {
      ...
      `;
      const metrics = { originality_max: 0, word_count: mockHtml.replace(/<[^>]+>/g,' ').split(/\s+/).filter(Boolean).length };
    ...
    const fetchedHtml: string = (typeof fetched === 'string')
      ? fetched
    ...
    if (Array.isArray(outlineResp)) {
      ...
      } else if (typeof outlineResp.h2 === 'string' || typeof outlineResp.title === 'string') {
        iterableOutline = [outlineResp];
      } else if (outlineResp.content && Array.isArray(outlineResp.content)) {
        iterableOutline = outlineResp.content;
      } else {
        iterableOutline = Object.keys(outlineResp).map(k => ({ h2: k, text: outlineResp[k] }));
      }
    }
...
iterableOutline = iterableOutline.map((item: any) => {
  ...
  if (typeof item === 'object') {
    ...
    const raw = item.h2 ?? item.title ?? item.heading ?? item.h ?? item.heading_text ?? '';
  ...
  return { h2: cleanHeading(item) };
    ...
    if (!Array.isArray(iterableOutline) || iterableOutline.length === 0) {
      iterableOutline = [{ h2: brief.title || brief.heading || 'Article' }, { h2: 'Overview' }, { h2: 'Conclusion' }];
      console.warn('normalizeOutline: outlineResp was empty or unrecognized — using fallback skeleton');
    } else {
      console.log('normalizeOutline: produced', iterableOutline.length, 'sections (sample h2):', iterableOutline.slice(0,3).map(s=>s.h2));
    }
    ...
    console.log('DEBUG: draft keys ->', draft && typeof draft === 'object' ? Object.keys(draft) : typeof draft);

    // 6. Normalize draft -> html string
    const responseHtml = normalizeDraftToHtml(draft);

    // 7. Metrics (simple)
    const genText = responseHtml.replace(/<[^>]+>/g,' ').split(/\s+/).filter(Boolean);
    let maxSim = 0;
    try {
      const genParagraphs = responseHtml.split(/<\/p>|<\/h2>|<\/h1>/i).map((s:string)=>s.replace(/<[^>]+>/g,'').trim()).filter(Boolean);
      const compParagraphs = (extracted.paragraphs || []).map((p:string)=>String(p));
