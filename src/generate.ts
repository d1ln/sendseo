...
function coerceSectionToHtml(sec: any): string {
  ...
  if (typeof sec === 'object') {
    ...
    for (const k of cands) {
      ...
      if (Array.isArray(v) && v.length) return v.map(item => coerceSectionToHtml(item)).join('\n');
    ...
    if (Array.isArray((sec as any).sections) && (sec as any).sections.length) return (sec as any).sections.map((s:any)=>coerceSectionToHtml(s)).join('\n');
    if (Array.isArray((sec as any).blocks) && (sec as any).blocks.length) return (sec as any).blocks.map((s:any)=>coerceSectionToHtml(s)).join('\n');
    const heading = sec.heading || sec.title || sec.h2 || '';
    const body = sec.html || sec.body || sec.text || sec.content || '';
    if (heading || body) {
      const head = heading ? `<h2>${escapeHtmlMinimal(String(heading))}</h2>` : '';
      const bodyStr = typeof body === 'string' ? body : escapeHtmlMinimal(JSON.stringify(body));
      return head + `<div>${bodyStr}</div>`;
    }
    try { return `<pre>${escapeHtmlMinimal(JSON.stringify(sec, null, 2))}</pre>`; } catch { return escapeHtmlMinimal(String(sec)); }
  }
...
export async function generateSectionWrapper(h2: string, context: any) {
  ...
  if (client && typeof client.generateSection === 'function') {
    try {
      ...
      console.warn('generateSectionWrapper LLM error:', e instanceof Error ? e.message : String(e));
    }
  }
  // fallback stub
...
export async function assembleDraft(outline: any[], brief: any) {
  ...
  for (const s of outline) {
    const h2 = typeof s === 'string' ? s : (s.h2 || s.title || s.heading || String(s));
    // call wrapper
    ...
    const sanitized = html.replace(/<a\b[^>]*>/gi, '').replace(/<\/a>/gi, '').replace(/https?:\/\/\S+/gi, '').replace(/www\.\S+/gi, '');
    sections.push(sanitized);
  }
  const body = sections.join('\n');

  // Key takeaways: prefer brief.key_points or empty fallback
  const takeaways = (brief && (brief.key_points || brief.key_takeaways || brief.recommended_key_takeaways)) || [];
  let takeawaysHtml = '';
  if (Array.isArray(takeaways) && takeaways.length) {
    takeawaysHtml = `<h4 id="key-takeaways">KEY TAKEAWAYS</h4><ul>${takeaways.slice(0,5).map((t:any)=>`<li>${escapeHtmlMinimal(String(t))}</li>`).join('')}</ul>`;
  }
  ...
  const hasHow = (outline || []).some((o:any) => String((o.h2||o.title||o).toLowerCase()).includes('how') || String((o.h2||o.title||o).toLowerCase()).includes('prepare'));
  const prepHtml = hasHow ? '' : `<h2>How to prepare</h2><ol><li>Audit senders and services.</li><li>Validate SPF/DKIM records.</li><li>Monitor DMARC reports.</li></ol>`;
  ...
  return { title: brief?.title || 'Untitled', meta: brief?.meta || '', body_html: finalBody };
}
