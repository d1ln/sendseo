...
function analyzeForBrowserNeed(html: string | null | undefined): boolean {
  ...
  if (txt.includes('__next_data__') || txt.includes('window.__INITIAL_STATE__') || txt.includes('id="__nuxt"')) return true;
...
function extractTitleFromHtml(raw: string | null | undefined): string | undefined {
  if (!raw) return undefined;
  try {
    const $ = cheerio.load(raw);
    const t = $('meta[property="og:title"]').attr('content') || $('title').first().text() || $('h1').first().text();
  ...
  for (const url of urls) {
    ...
    try {
      ...
      try {
        ...
        html = typeof h === 'string' ? h : (h && (h as any).html ? (h as any).html : h ? String(h) : null);
      } catch (e) {
        html = null;
      }
      ...
      if (!html || needBrowser) {
        ...
        html = typeof res === 'string' ? res : (res as any).html || (res as any).data || null;
        row.usedFallback = true;
      }
