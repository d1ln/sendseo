...
const DEFAULT_USER_AGENT = 'Mozilla/5.0 (compatible; SeoBlogBot/1.0; +https://example.com/bot)';

export async function fastFetch(url: string, timeout = 8000): Promise<string> {
  const res = await axios.get(url, {
    headers: { 'User-Agent': process.env.FETCH_USER_AGENT || DEFAULT_USER_AGENT },
    timeout,
    responseType: 'text',
...
async function runPlaywrightFallback(url: string, waitForSelector = 'article, .post-content, .entry-content'): Promise<string> {
  ...
  try {
    ...
    throw new Error('Playwright not installed. Install with `npm i -D playwright` or set PLAYWRIGHT_ENABLED=false');
  }
  ...
  try {
    ...
    try { await page.waitForSelector(waitForSelector, { timeout: 3500 }); } catch(e) {}
 ...
 */
export async function fetchWithFallback(url: string): Promise<{ html: string } | string> {
  // 1. fast HTTP fetch
  let rawHtml = '';
  try {
    rawHtml = await fastFetch(url);
  } catch (err) {
    console.warn('fastFetch failed, will try Playwright if enabled:', err && (err as Error).message);
    rawHtml = '';
  }
  ...
  try {
    if (rawHtml && rawHtml.length > 200) {
      ...
      const mainHtml = extracted && extracted.main_html ? extracted.main_html : '';
      ...
      const isMainShort = !mainHtml || mainHtml.replace(/\s+/g,'').length < 120;
      const placeholderImg = images.length > 0 && images.every(img => /data:image\/gif|base64,iVBORw0KGgo/.test(img.src) || img.src.trim()==='');
    ...
    console.warn('extractStructure check failed, will consider Playwright fallback:', e && (e as Error).message);
  }

  // 3. Playwright fallback (if allowed by env)
  const enablePw = process.env.PLAYWRIGHT_ENABLED === 'true' || (!!process.env.PLAYWRIGHT_ENABLED && process.env.PLAYWRIGHT_ENABLED !== 'false');
  if (!enablePw) {
