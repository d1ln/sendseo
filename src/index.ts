...
async function run(url: string) {
  ...
  const htmlSource = (typeof fetched === 'string') ? fetched : (fetched && (fetched as any).html ? (fetched as any).html : '');
  if (!htmlSource) {
  ...
  const iterableOutline = Array.isArray(outlineResp) ? outlineResp : (Array.isArray((outlineResp as any).outline) ? (outlineResp as any).outline : []);

  if (!iterableOutline || iterableOutline.length === 0) {
    console.warn('generateOutline returned empty; creating fallback outline from brief.');
  }
  console.log('Normalized outline (sample):', iterableOutline.slice(0, 5).map((s:any, i:number) => {
  ...
  console.log('Draft title:', draft && (draft.title || brief.title) || 'Untitled');
  ...
  const bodyHtml = (typeof draft === 'string')
    ? draft
    : (draft && ((draft as any).body_html || (draft as any).html || (draft as any).body)
      ? ((draft as any).body_html || (draft as any).html || (draft as any).body)
      : '');
  if (!bodyHtml) {
    console.warn('assembleDraft produced no body_html; raw draft object keys:', draft ? Object.keys(draft) : 'none');
  } else {
  ...
  try {
    const generatedParagraphs = (bodyHtml || '').split(/<\/p>|<\/h2>|<\/h1>/i).map((s: string) => s.replace(/<[^>]+>/g,'').trim()).filter(Boolean);
    const sourceParagraphs = (extracted.paragraphs || []).map((p:any) => String(p).trim()).filter(Boolean);
    ...
    for (const gp of generatedParagraphs) {
      for (const sp of sourceParagraphs) {
        ...
        if (!passesSimilarityThreshold(sim)) {
          ...
          console.warn('Similarity flag — potential overlap; score:', sim.toFixed(3));
        }
      }
    }
  ...
  console.log('Draft assembled successfully. You can inspect draft.body_html or wire to the server UI.');
...
run(argv[0]).then(res => {
  ...
  if (res && res.draft) {
    ...
    const sample = (res.draft.body_html || (res.draft as any)?.html || '').slice(0, 300).replace(/\n+/g,' ');
